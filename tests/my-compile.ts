    ...
    try {
        ...
        if (config.lang === 'func') {
            let funcResult = await doCompileFunc({
                ...
                sources: (path) => {
                    // process.stdout.write('PATH: ' + CONTRACTS_FUNC_ROOT + numericFolder + '/' + path + '\n\n');
                    return fs.readFileSync(CONTRACTS_FUNC_ROOT + numericFolder + '/' + path, 'utf-8');
                },
                optLevel: 2,
            });
        ...
        process.stdout.write('WOT ERR: ' + (ex as any).toString());
        process.exit(1);
    }
}
