...
class GasReporter {
    ...
    printTableOldVsNew(table) {
        ...
        for (let { column, old, cur, diff } of table) {
            ...
            if (column.startsWith('code size')) {
                if (old) {
                    s += `${old} `;
                    if (diff) s += colors.greenBright(`→ ${cur}`);
                    else if (old !== cur) s += `→ ${cur}`;
                    else s += `=`;
                } else {
                    s += cur;
                }
                ...
                if (old) {
                    s += `${old.padEnd(7)} `;
                    if (diff < 0) s += colors.red(`→ ${cur.padEnd(7)} +${(-diff).toFixed(2)}%`);
                    else if (diff > 0) s += colors.greenBright(`→ ${cur.padEnd(7)} -${diff.toFixed(2)}%`);
                    else s += `=`;
                } else {
                    s += cur;
                }
    ...
    prepareTableOldVsNew(oldRun, lastRun) {
        ...
        for (let key in lastRun.codeSize) {
            // {"xxx bits": n, "xxx cells": m}
            if (!key.endsWith(' bits'))
                // handle only "bits", join bits+cells to one line
                continue;
    ...
    persistLastRunIntoBenchFile(/**string*/ numericFolder, /**string*/ commitDesc) {
        const lastRun = this.parseLastRunForContract(numericFolder);
        if (lastRun === null) throw `lastRun is null for ${numericFolder}`;
