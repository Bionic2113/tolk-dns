...
async function cmdSave() {
    ...
    const withLastRun = allContracts
        .filter((c) => gasReporter.parseLastRunForContract(c.numericFolder))
        .map((c) => c.numericFolder);
    ...
    if (withLastRun.length === 1) {
        ...
        numericFolder = await promptNumericFolder(
            allContracts.map((c) => c.numericFolder),
            true,
        );
    }
...
async function cmdShow() {
    ...
    let numericFolder = await promptNumericFolder(
        allContracts.map((c) => c.numericFolder),
        true,
    );
...
async function cmdGenerateReadme() {
    ...
    if (pos1 === -1 || pos2 === -1 || pos2 < pos1) throw 'Could not find positions in README.md';
